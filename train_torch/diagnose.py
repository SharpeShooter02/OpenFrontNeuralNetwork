...
def pad(rows, K, F):
    c = torch.zeros(K, F); m = torch.zeros(K)
    for i, row in enumerate(rows[:K]):
        c[i] = torch.tensor(row, dtype=torch.float32); m[i] = 1.0
    return c, m

def choose(obs, cands, ptiles):
    x = torch.tensor(obs, dtype=torch.float32)
    logits, troop_mean = net(x)
    a = int(Categorical(logits=logits).sample().item())
    cpad, cmask = pad(cands, KCAND, FCAND)
    tgt = int(Categorical(logits=net.score_cands(cpad).masked_fill(cmask == 0, -1e9)).sample().item())
    ppad, pmask = pad(ptiles, KP, FP)
    oh = torch.zeros(NTYPE)
    if a in PLACE_MAP: oh[PLACE_MAP[a]] = 1.0
    feats = torch.cat([ppad, oh.expand(KP, NTYPE)], dim=-1)
    ptg = int(Categorical(logits=net.score_place(feats).masked_fill(pmask == 0, -1e9)).sample().item())
    return a, float(torch.sigmoid(troop_mean).item()), tgt, ptg
...
for ep in range(3):
    r0 = rpc({"cmd": "reset", "seed": 90001 + ep}); o = r0["obs"]; cands = r0["cands"]; ptiles = r0["ptiles"]
    hist = [0]*N_ACT; troops_used = []; diplo = {"accept": 0, "request": 0, "break": 0}
    place = {"defense": [], "factory": []}                            # frontline feature of chosen tile, by type
    done, steps, total = False, 0, 0.0
    print(f"\n===== EPISODE {ep} (held-out seed {90001+ep}) =====")
    print(f"  start: {decode(o)}")
    while not done and steps < 2000:
        a, tr, tgt, ptg = choose(o, cands, ptiles); hist[a] += 1; troops_used.append(tr)
        ...
        if a in PLACE_MAP and ptg < len(ptiles):                    # frontline feature (idx 0) of the chosen tile
            if a == 6: place["defense"].append(ptiles[ptg][0])
            elif a == 12: place["factory"].append(ptiles[ptg][0])
        r = rpc({"cmd": "step", "action": a, "troop": tr, "target": tgt, "ptarget": ptg})
    ...
    df = sum(place['defense'])/len(place['defense']) if place['defense'] else float('nan')
    ff = sum(place['factory'])/len(place['factory']) if place['factory'] else float('nan')
    print(f"  placement frontline-rate: defense={df:.2f} (want high) | factory={ff:.2f} (want low)  [n def={len(place['defense'])} fac={len(place['factory'])}]")
